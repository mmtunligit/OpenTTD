...
/* static */ bool ScriptRoad::_BuildRoadStationInternal(TileIndex tile, TileIndex front, RoadVehicleType road_veh_type, bool drive_through, StationID station_id)
{
	...
	return ScriptObject::Command<CMD_BUILD_ROAD_STOP>::Do(tile, 1, 1, stop_type, drive_through, entrance_dir, ScriptObject::GetRoadType(), std::vector<RoadStopClassID>{ROADSTOP_CLASS_DFLT}, std::vector<uint16_t>{0}, to_join, station_id != ScriptStation::STATION_JOIN_ADJACENT);
}
